...
# ============
#
# Tasks plug into the scheduler.  They are either active or pending.  Pending
# tasks have some sort of trigger.  When that trigger fires, the scheduler 
# makes the task active.
#
# As long as there are active tasks, the scheduler executes their run
# function.  Active tasks are executed strictly according to their priority.
# If they don't specify one, the default is 100.
#
# When they run, they are given the a parameter that references what has
# caused them to wake up.  This is either a trigger or None if the task is
# active.
#
...
# Note that it is possible to register a trigger without going to sleep.
# This currently doesn't make any sense, but it may be possible to use it
# to create recurrent, persistent triggers.
#
...
# Pending tasks are added via the "add_pending" function, which is called
# with the trigger.  It is prefectly safe to schedule more than one task one

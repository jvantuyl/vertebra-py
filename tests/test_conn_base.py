...
class test_conn_base(mocked_conn_base):
  MOCK_TYPE = mock

  def test_mock_didnt_crash(self):
    """conn.base: did it crash?"""
    assert not self.mock.crashed

  def test_mock_didnt_die(self):
  ...
  def test_mock_stopped(self):
    """conn.base: did we stop?"""
    assert self.mock.stopped

class test_conn_base_broken(mocked_conn_base):
